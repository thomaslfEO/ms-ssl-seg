...
from repo_paths import ssl_fastsiam_checkpoint, ssl_moco_swin_checkpoint  # noqa: E402
...
def _require_fastsiam_checkpoint() -> Path:
    path = ssl_fastsiam_checkpoint()
    if path is None:
        raise ValueError(
            "Set SSL_FASTSIAM_CHECKPOINT when using FastSiam pretrained weights."
        )
    if not path.is_file():
        raise FileNotFoundError(f"FastSiam checkpoint not found: {path}")
    return path
...
class MyModel(L.LightningModule):
    ...
    def _prepare_model(self):
        ...
        if self.weights not in ['imagenet', None]:
            ...
            if self.weights == 'Sentinel_2_TOA_Resnet18':
            ...
            elif self.weights in ("msuavNPlus_Swin", "msuav_SWIN"):
                ...
                if self.architecture == 'DPT':
                    ...
                    if self.weights == "msuavNPlus_Swin":
                        self.backbone = 'tu-swin_s3_tiny_224'
                        path = str(_require_moco_swin_checkpoint())
                    else:  # msuav_SWIN
                        self.backbone = 'tu-swin_small_patch4_window7_224'
                        path = str(_require_fastsiam_checkpoint())

                    #79 epoch mocov3 (kept as reference comment)
...
def dl_train(
    ...
    pretrained_weights_options = [
        'imagenet',               # 0
        None,                     # 1
        'Sentinel_2_TOA_Resnet18', # 2
        'Sentinel_2_TOA_Resnet50', # 3
        'msuavNPlus_Swin',        # 4 — tu-swin_s3_tiny_224 + MoCo-v3 weights
        'msuav_SWIN',             # 5 — tu-swin_small_patch4_window7_224 + FastSiam weights
    ]
    ...
    elif pretrained_weights == 'msuavNPlus_Swin':
        backbone = 'tu-swin_s3_tiny_224'

    elif pretrained_weights == 'msuav_SWIN':

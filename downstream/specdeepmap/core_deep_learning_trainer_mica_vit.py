...
from repo_paths import ssl_mae_vit_base_checkpoint, ssl_mae_vit_checkpoint  # noqa: E402


def _is_vit_base_backbone(backbone: str) -> bool:
    return "vit_base" in backbone.lower()


def _require_mae_vit_checkpoint(backbone: str) -> Path:
    is_base = _is_vit_base_backbone(backbone)
    path = ssl_mae_vit_base_checkpoint() if is_base else ssl_mae_vit_checkpoint()
    env_name = "SSL_MAE_VIT_BASE_CHECKPOINT" if is_base else "SSL_MAE_VIT_CHECKPOINT"
    if path is None:
        raise ValueError(
            f"Set {env_name} when using MAE ViT SSL weights (pretrained_weights index 4)."
        )
    if not path.is_file():
        raise FileNotFoundError(f"MAE ViT checkpoint not found: {path}")
    return path
        ...
        if self.weights not in ['imagenet', None]:
            ...
            if self.weights == 'Sentinel_2_TOA_Resnet18':
                ...
                if self.architecture == 'DPT':
                    checkpoint_path = str(_require_mae_vit_checkpoint(self.backbone))
